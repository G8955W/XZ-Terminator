...
function PainTransfer() {
  ...
  const handleCopyResult = () => {
    ...
    const template = isZh
      ? `我已经用选择终结者帮你决定了，你今天必须【${friendDecision}】！不服憋着 😎`
      : `I have decided your fate using Decision Terminator: You MUST ${friendDecision} today! Deal with it 😎`
      ...
      <div className="min-h-screen bg-notion-dark text-notion-text overflow-hidden">
        ...
        <div className="flex flex-col items-center justify-center min-h-[calc(100vh-60px)] px-4">
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.98 }}
            onClick={() => navigate('/')}
            className="mt-8 px-6 py-3 bg-gradient-to-r from-yellow-600 to-orange-600 text-white rounded-xl font-semibold hover:shadow-xl hover:shadow-yellow-500/20 transition-all"
          >
            {t('back-to-menu')}
          </motion.button>
